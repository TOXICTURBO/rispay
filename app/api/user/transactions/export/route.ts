...
export const GET = withAuth(async (req: NextRequest, user) => {
  ...
  if (format === 'csv') {
    ...
    const rows = transactions.map((tx) => {
      ...
      return [
        ...
        Number(tx.bank_fee).toFixed(2),
        Number(tx.system_tax).toFixed(2),
        Number(tx.amount) + Number(tx.bank_fee) + Number(tx.system_tax),
        otherParty,
        tx.status,
        tx.memo || '',
        tx.tag || '',
  ...
  const jsonData = transactions.map((tx) => {
    ...
    return {
      ...
      bankFee: Number(tx.bank_fee),
      tax: Number(tx.system_tax),
      totalDeducted: Number(tx.amount) + Number(tx.bank_fee) + Number(tx.system_tax),
      otherParty: isSent
        ? tx.receiver_account.user.username
        : tx.sender_account.user.username,
      status: tx.status,
